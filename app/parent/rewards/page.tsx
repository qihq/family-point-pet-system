...
export default function ParentRewardsPage(){
  ...
  const [cacheV] = useState<string>(()=>String(Date.now()));
                  ...
                  {rows.map(r=> (
                    <tr key={r.id} className="border-t">
                      <td className="px-4 py-2">
                        {r.imageUrl ? (<img src={`${r.imageUrl}${r.imageUrl.includes('?') ? '&' : '?' }v=${cacheV}`} alt={r.name} className="w-12 h-12 object-cover rounded" />) : "-"}
                      </td>
                      ...
                      <td className="px-4 py-2">
                        <span className={r.enabled? "px-2 py-0.5 rounded-full text-xs bg-green-100 text-green-700" : "px-2 py-0.5 rounded-full text-xs bg-gray-100 text-gray-700"}>{r.enabled? `启用`:`禁用`}</span>
                      </td>
                      <td className="px-4 py-2">
                        <button onClick={()=>openEdit(r)} className="text-blue-600 hover:text-blue-800">{`编辑`}</button>
                        <button onClick={()=>toggle(r)} className="ml-3 text-amber-600 hover:text-amber-800">{`启/停`}</button>
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-5">
        ...
        <div className="mt-3 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="text-sm text-gray-700 block">{`库存（空=不限制）`}
              <input type="number" value={stock==null? "": String(stock)} onChange={e=>{ const v=e.target.value; setStock(v===''? null: parseInt(v)); }} className="mt-1 w-full border rounded px-3 py-1.5" />
            </label>
          </div>
