...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let pet = await prisma.pet.findUnique({ where: { childId }, select: petCoreSelect });
    if (!pet) {
      pet = await prisma.pet.create({ data: { childId, name: "小暖暖" }, select: petCoreSelect });
    }
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "获取宠物信息失败" },
      { status: 500 }
    );
  }
}
