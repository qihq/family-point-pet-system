...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const reason = String(body.reason || "手动扣分");
    ...
    const result = await spendPoints({ childId, amount, sourceType: SourceType.manual, description: reason });
    return NextResponse.json({ success: true, message: "已扣分", data: result });
  } catch (error: any) {
    console.error("手动扣分失败", error);
    const status = error.message?.includes("积分不足") ? 400 : 500;
